...
const CompanyForm: React.FC<CompanyFormProps> = ({ company, setIsDirectorSelected }) => {
  ...
  const handleContinue = () => {
    if (!selectedDirector) {
      setDirectorError(t("main.ErrorMsg"));
      return;
    }
  ...
  return (
    <div className="border border-gray-300 p-6 bg-white w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <InputField label={t("RegBussOrg.CompanyDetails.ClassOfCom")} placeholder="Enter class" value={company.classOfCompany} readonly   />

        <InputField label={t("RegBussOrg.CompanyDetails.ComType")} placeholder="Enter type" value={company.type} readonly />
      </div>
