...
const DirectorAadhar: React.FC<DirectorAadharProps> = ({ onCompleted }) => {
  ...
  const handleValidate = () => {
    if (aadhar) {
      setIsValidAadhar(!isValidAadhar);
  ...
  return (
    <div className="flex flex-col md:flex-row items-start gap-8 border border-gray-300 w-full p-6 rounded-md shadow-sm">
      {/* Left Section */}
      <div className="md:w-1/3 flex flex-col justify-between pr-6 md:border-r border-gray-300">
        <div>
          <h2 className="text-lg font-semibold mb-4">
            {t("RegBussOrg.DirectorPAN.Heading")}
          </h2>
          <p className="text-sm text-gray-700">
            <span className="font-medium">
              {t("RegBussOrg.DirectorPAN.DINNumberLabel")}:
            </span>{" "}
            9012345
          </p>
          <p className="text-sm text-gray-700">
            <span className="font-medium">
              {t("RegBussOrg.DirectorPAN.FullNameLabel")}:
            </span>{" "}
            Rahul Raghuwanshi
          </p>
          <p className="text-sm text-gray-700">
            <span className="font-medium">
              {t("RegBussOrg.DirectorPAN.NationalityLabel")}:
            </span>{" "}
            India
          </p>
        </div>
      ...
      {/* Right Section */}
      <div className="md:w-2/3 flex flex-col gap-6 md:pl-6">
        {/* Aadhaar Row */}
        <div className="flex flex-col md:flex-row gap-6">
          ...
          {/* Validate Button */}
          <div className="flex flex-col md:mt-4 justify-start">
